...
let directionalLight; // Store directional light reference
...
function init() {
  ...
  directionalLight = new THREE.DirectionalLight(0xffffff, 0.8); // Assign to the higher-scoped variable
  directionalLight.position.set(10, 20, 10);
  directionalLight.castShadow = true;
  directionalLight.shadow.mapSize.width = 2048;
  directionalLight.shadow.mapSize.height = 2048;

  // Configure the shadow camera frustum to cover a larger area
  directionalLight.shadow.camera.near = 0.5;
  directionalLight.shadow.camera.far = 500; // Increased to see further
  directionalLight.shadow.camera.left = -100; // Increased horizontal coverage
  directionalLight.shadow.camera.right = 100;
  directionalLight.shadow.camera.top = 100; // Increased vertical coverage
  directionalLight.shadow.camera.bottom = -100;
  directionalLight.shadow.camera.updateProjectionMatrix(); // Apply changes
...
function update() {
  ...
  // Update directional light to follow the ball for consistent shadows
  if (ball && directionalLight) {
    directionalLight.position.set(
      ball.position.x + 10,
      ball.position.y + 20,
      ball.position.z + 10
    );
    directionalLight.target = ball; // Make the light look at the ball
    // No need to add target to scene if it's an object already in the scene like the ball
    // directionalLight.target.updateMatrixWorld(); // Ensure target's matrix is updated
  }
